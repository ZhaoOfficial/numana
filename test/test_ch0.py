...
class TestNest(object):
    ...
    def test_nest(self):
        self.output_nest(0.5,   [-1, 5, -3, 3, 2],   [0, 0, 0, 0])
        self.output_nest(0.5,   [-1, 5, -3, 3, 2])
        self.output_nest([-2, -1, 0, 1, 2], [-1, 5, -3, 3, 2])
        self.output_nest(1,     [1, 0.5, 0.5, -0.5], [0, 2, 3])

        self.output_nest(1 / 3, [1, 1, 5, 1, 6])
        self.output_nest(1 / 3, [1, -5, 5, 4, -3])
        self.output_nest(1 / 3, [1, 0, -1, 1, 2])

        self.output_nest(-0.5,  [7, -3, -2, 6])
        self.output_nest(-0.5,  [1, -3, 1, -3, -1, 8])
        self.output_nest(-0.5,  [4, -2, 0, 0, -2, 0, 4])

        self.output_nest(0.5,   [1, 0, 2, 0, -4, 0, 1])
        self.output_nest(0.25,  [1, 2, -4, 1])

        self.output_nest(5,     [1, 0.5, 0.5, -0.5], [0, 2, 3])
        self.output_nest(-1,    [1, 0.5, 0.5, -0.5], [0, 2, 3])
        self.output_nest(0.5,   [4, 4, 1, 3, 2],     [0, 1, 2, 3])
        self.output_nest(-0.5,  [4, 4, 1, 3, 2],     [0, 1, 2, 3])

        self.output_nest(1.00001, np.ones(51, dtype=int))
        print("\033\13334m[{}]\033\1330m.".format((1.00001 ** 51 - 1) / (1.00001 - 1)))

        self.output_nest(1.00001, np.array([1, -1] * 50))
...
class TestBinaryDecimal(object):
    def output_dec2bin(self, deci: Union[float, str], fraction_len: int = 10):
    ...
    def output_bin2dec(self, bina: str, recur_pos: tuple = None):
...
if __name__ == "__main__":
    pytest.main(["-s", "test_ch0.py::TestNest::test_nest"])
